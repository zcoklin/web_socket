 ...
 * For information on when to used fragmented frames see
 * http://tools.ietf.org/html/rfc6455#section-5.4<br>
 * Fragmented and normal messages can not be mixed. One is however allowed to
 * mix them with control messages like ping/pong.
 *
...
public class FragmentedFramesExample
{
    public static void main(String[] args) throws URISyntaxException, IOException, InterruptedException
    {
        ...
        WebSocketClient websocket = new ExampleClient(new URI("ws://localhost:8887"), new Draft_17()); // Draft_17
                                                                                                       // is
                                                                                                       // implementation
                                                                                                       // of
                                                                                                       // rfc6455
        ...
        while (websocket.isOpen())
        {
            ...
            for (int position = 2;; position += 2)
            {
                if (position < longelinebuffer.capacity())
                {
                    ...
                    websocket.sendFragmentedFrame(Opcode.TEXT, longelinebuffer, false);// when
                                                                                       // sending
                                                                                       // binary
                                                                                       // data
                                                                                       // one
                    ...
                    // after calling sendFragmentedFrame one may reuse the
                    // buffer given to the method immediately
                }
                else
                {
                    longelinebuffer.limit(longelinebuffer.capacity());
                    websocket.sendFragmentedFrame(Opcode.TEXT, longelinebuffer, true);// sending
                                                                                      // the
                                                                                      // last
                                                                                      // frame
                    break;
